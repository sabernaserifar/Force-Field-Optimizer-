...
import write_rexpon                                               # ...
import write_morse, write_morse_pqeq                                               # ...
import write_lj_cut_coul_cut
import write_ljcut                                               # ...
import write_exryd, write_exrydc                                 # ...
import write_rexpon_unb
import write_prld                                               # ...
import write_opls                                                # ...
...
def prepare_eng_table(ff, polar):
    ...
    if not polar:
        ...
        elif ff.get_name().lower() == 'prld':
            logging.info('Writing energy_table.txt output...')
            h = open('energy_table.txt', 'w')
            h.write("%-30s  ETOTAL  PE  ELECTRO\n" % ('#str'))
        elif ff.get_name().lower() == 'rexpon_unb':
            logging.info('Writing energy_table.txt output...')
            h = open('energy_table.txt', 'w')
            h.write("%-30s  ETOTAL  PE  ELECTRO\n" % ('#str'))
...
def write_in_file(ff, data, polar):
    ...
    if ff_name == 'uffqm':                                   # Write the data.in file
    ...
    elif ff_name == 'morse_pqeq':
        write_morse_pqeq.write_in(ff, data)                    #  ... morse_pqeq
    elif ff_name == 'lj/cut/coul/cut':
        write_lj_cut_coul_cut.write_in(ff, data)                    #  ... morse_pqeq
    elif ff_name == 'ljcut':
        ...
        write_prld.write_in(ff, data)
    elif ff_name == 'rexpon_unb':
        write_rexpon_unb.write_in(ff, data)
...
def write_eng_table(lmp, h, name, data):
    ...
    if name == 'uffqm':
        ...
        pe      = lmp.extract_compute("eng", 0, 0)               # Potential energy
        electro = lmp.extract_variable("epqeq", 0, 0)            # Electrostatic energy
        evdw = lmp.extract_variable("evdw", 0, 0) 

        h.write('%-30s %8.4f %8.4f %8.4f %8.4f \n' % (data.split("data_lammps_")[1], etotal,pe,electro,evdw))
...
def run(ffield, params, train, geo):
    ...
    logging.basicConfig(filename='log.opt', filemode='w', level=logging.DEBUG)
    ...
    if geo != None:                                              # Create data files if geo file supplied
        ...
        if name == 'uffqm':
            ...
            geotodatalammps.data_file_complete(geo)
        elif name == 'morse_pqeq':
            geotodatalammps.data_file_complete(geo)
        elif name == 'lj/cut/coul/cut':
            geotodatalammps.data_file_complete(geo)
        elif name == 'ljcut':
            geotodatalammps.data_file_simple(geo)
        elif name == 'exryd':
    ...
    if method == '0':                                   # Run without optimization
        ...
        if algorithm == 'slsqp':
            ...
            res = minimize(optimizer, init, args=(data_read, train_read, ff_read, param_read.get_bounds(), polar), method='Nelder-Mead')
#            res = minimize(optimizer, init, args=(data_read, train_read, ff_read, param_read.get_bounds(), polar), method='Nelder-Mead', options={'fatol':0.01})
        ...
        write_out(ff_read, res[0], param_read.get_bounds())       # Create the fort.4 result force field

        # Create a fort.99 type file using optimized values
        optimizer(res[0], data_read, train_read, ff_read, param_read.get_bounds(), polar, res_print=True, debugging=False)
