...
# Assuming the force field is rexpon/unb
...
def write_in(ffield, data_file):
    ...
    g.write('# rexpon/unb - generated by optimizer\n')
    g.write('units            ' + units + '\n')
    ...
    g.write('\n')
    g.write('pair_style       hybrid/overlay coul/pqeqgauss 0.00 12.0 rexpon/unb 0.0 12.0 \n')  # Change later
    #g.write('pair_style       hybrid/overlay coul/pqeqgauss 0.00 12.0 rexpon/unb 0.0 12.0  hbond/dreiding/morse 4 0.0 10.0 90\n')  # Change later

    #g.write('pair_style       hybrid/overlay rexpon/unb 0.0 12.5\n')  # Change later
    ...
    g.write('#                       D R0 L  \n')
    g.write('pair_coeff  * *  rexpon/unb  -1 0 0 \n')
    for i in range(len(dict['VDW'])):
        temp = dict['VDW'][i]
        g.write('pair_coeff ' + str(temp[0]) + ' ' + str(temp[1]) + ' rexpon/unb ' + ' '.join(map(str, temp[2:])) + '\n')
    g.write('\n')
    g.write('pair_modify pair rexpon/unb mix geometric\n')


    # HYDROGEN BOND STYLE
    #g.write('pair_coeff  * *  hbond/dreiding/morse  * i 0 0 0 0 0 0 0\n')
    #for i in range(len(dict['HYDROGEN_BOND'])):
        #temp = dict['HYDROGEN_BOND'][i]
        #g.write('pair_coeff ' + str(temp[0]) + ' ' + str(temp[2]) + ' hbond/dreiding/morse ' + ' ' + str(temp[1]) + ' ' +  str(temp[3]) + ' '+  ' '.join(map(str, temp[4:])) + '\n')



    g.write('neigh_modify one 100000 page 1000000\n')
    g.write('# CHECK MIXING RULE FOR rexpon/unb\n')
    g.write('\n')
    """
